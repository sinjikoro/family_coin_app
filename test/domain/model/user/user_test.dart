...
void main() {
  group('User', () {
    test('正常な値でUserを生成できる', () {
      final user = User(id: UserId.generate(), name: 'テストユーザー', familyCoinBalance: const FamilyCoin(1000));

      expect(user.id, isA<UserId>());
      expect(user.name, 'テストユーザー');
      expect(user.familyCoinBalance, const FamilyCoin(1000));
    });

    test('UserをJSONに変換できる', () {
      final user = User(id: UserId(1), name: 'テストユーザー', familyCoinBalance: const FamilyCoin(1000));
    ...
    test('Userをコピーできる', () {
      ...
      final copiedUser = user.copyWith(name: 'コピーされたユーザー', familyCoinBalance: const FamilyCoin(2000));
